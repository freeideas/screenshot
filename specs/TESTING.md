...
**AI Visual Verification of Common Applications:**

Use `visual-test.py` to verify screenshots match expected appearance:

1. Launch `cmd.exe`, capture screenshot, verify it looks like a command prompt
2. Launch `notepad.exe`, capture screenshot, verify it looks like Notepad
3. Always kill spawned processes when done

```bash
uv run --script ./the-system/scripts/visual-test.py ./tmp/screenshot.png "A Windows command prompt"
# Exit: 0 = passed, 1 = failed, 2 = error
```

Or as module: `passed, explanation = visual_test.check_visual(image_path, description)`
