Ensure all reasonably testable behaviors from `./README.md` and `./specs/*.md` are represented in `./reqs/`.
...
3. Identify reasonably testable behaviors in `./README.md` or `./specs/*.md` missing from flows
...
**DO create requirements for:**
- **Build output verification (WHAT must be in ./released/)** -- file existence, names, sizes, structure are testable
...
- **Build scripts or build processes (HOW to compile)** -- skip step-by-step build instructions
- **Development prerequisites** (.NET SDK, compilers, Java, Python, etc.)
- **How to compile or package (build process steps)** -- build instructions are not runtime requirements
...
- OK "Never block I/O" -> this IS a requirement (add it if missing)
- X "Will crash with OOM instead of blocking" -> this is an ILLUSTRATION of non-blocking (don't add OOM as requirement)
- OK "Use non-blocking I/O" -> this IS a requirement (architectural, testable via code review)
- X "Fast performance" -> this is NOT testable (skip it)
- X "Does not support UDP" -> this is NOT a requirement (absence of feature, nothing to test)
- OK "Returns error 'UDP not supported' if UDP attempted" -> this IS a requirement (observable error behavior)
- OK "Accepts one directory argument" -> this IS a requirement (describes happy path)
- X "Error if two directory arguments provided" -> skip unless README explicitly documents this error
- OK "Port number is required" -> this IS a requirement (describes correct usage)
- X "Exit with error if port missing" -> skip unless README explicitly documents this error behavior
- OK "./released/ must contain exactly 4 files" -> this IS a requirement (testable build output)
- OK "ConnectorWebApp.exe must be ~80-100MB" -> this IS a requirement (testable file size)
- X "Run dotnet publish -c released" -> this is NOT a requirement (build process step)
- X "Requires .NET 8 SDK to build" -> this is NOT a requirement (development prerequisite)
- X "Build will fail if a critical file is missing" -> this is NOT a requirement (natural consequence)
