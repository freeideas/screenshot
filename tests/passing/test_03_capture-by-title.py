...
"""
Test: Capture by Window Title
Verifies capture by window title works according to requirements.
...
def test_req_title_001_capture_by_title_flag():
    """$REQ_TITLE_001: Capture Window by Title Flag

    When --title <title> is specified, capture a window matching that title.
    If multiple windows share the same title, one will be captured (unspecified which).
    """
    print("Testing $REQ_TITLE_001: Capture Window by Title Flag...")
    ...
    output_path = os.path.join(tmp_dir, f"{timestamp}_capture_by_title.png")

    # Run screenshot.exe with --title flag
    result = subprocess.run(
    ...
    assert result.returncode == 0, \
        f"screenshot.exe --title failed (exit code {result.returncode})\nstderr: {result.stderr}"
    ...
    print("  $REQ_TITLE_001: Capture Window by Title Flag works correctly")
    return True


def test_req_title_002_save_to_png():
    """$REQ_TITLE_002: Save to Specified PNG File

    When a .png file path is specified as output, save the screenshot to that exact location.
    """
    print("Testing $REQ_TITLE_002: Save to Specified PNG File...")
    ...
    timestamp = time.strftime("%Y-%m-%d-%H-%M-%S")
    output_path = os.path.join(tmp_dir, f"{timestamp}_exact_path_test_title.png")

    # Run screenshot.exe with --title flag and specific output path
    result = subprocess.run(
        [exe_path, "--title", window_title, output_path],
        capture_output=True,
        text=True,
    ...
    print("  $REQ_TITLE_002: Save to Specified PNG File works correctly")
    return True


def test_req_title_003_output_success_message():
    """$REQ_TITLE_003: Output Success Message

    When a screenshot is successfully captured, output 'Wrote [filepath]' before exiting.
    """
    print("Testing $REQ_TITLE_003: Output Success Message...")
    ...
    timestamp = time.strftime("%Y-%m-%d-%H-%M-%S")
    output_path = os.path.join(tmp_dir, f"{timestamp}_message_test_title.png")
    ...
    print("  $REQ_TITLE_003: Output Success Message works correctly")
    return True
...
def test_visual_verification():
    ...
    timestamp = time.strftime("%Y-%m-%d-%H-%M-%S")
    output_path = os.path.join(tmp_dir, f"{timestamp}_visual_test_title.png")
...
def main():
    ...
    try:
        ...
        test_req_title_001_capture_by_title_flag()
        test_req_title_002_save_to_png()
        test_req_title_003_output_success_message()
