...
Verifies capture by window ID works according to requirements.
...
def test_req_id_001_capture_by_id_flag():
    """$REQ_ID_001: Capture Window by ID Flag

    When --id <window-id> is specified, capture a window by its alphanumeric window ID.
    """
    print("Testing $REQ_ID_001: Capture Window by ID Flag...")
    ...
    # Verify window ID is alphanumeric (hexadecimal)
    assert re.match(r'^[A-Fa-f0-9]+$', window_id), \
        f"Window ID '{window_id}' is not alphanumeric hexadecimal"

    print(f"  Using window: id={window_id}, title=\"{test_window['title']}\"")
    ...
    output_path = os.path.join(tmp_dir, f"{timestamp}_capture_by_id.png")

    # Run screenshot.exe with --id flag
    result = subprocess.run(
        [exe_path, "--id", window_id, output_path],
        capture_output=True,
        text=True,
        encoding='utf-8',
        timeout=30
    ...
    assert result.returncode == 0, \
        f"screenshot.exe --id failed (exit code {result.returncode})\nstderr: {result.stderr}"
    ...
    print("✓ $REQ_ID_001: Capture Window by ID Flag works correctly")
    return True


def test_req_id_002_save_to_png():
    """$REQ_ID_002: Save to Specified PNG File

    When a .png file path is specified as output, save the screenshot to that exact location.
    """
    print("Testing $REQ_ID_002: Save to Specified PNG File...")
    ...
    timestamp = time.strftime("%Y-%m-%d-%H-%M-%S")
    output_path = os.path.join(tmp_dir, f"{timestamp}_exact_path_test.png")

    # Run screenshot.exe with --id flag and specific output path
    result = subprocess.run(
        [exe_path, "--id", window_id, output_path],
        capture_output=True,
        text=True,
    ...
    print("✓ $REQ_ID_002: Save to Specified PNG File works correctly")
    return True


def test_req_id_003_output_success_message():
    """$REQ_ID_003: Output Success Message

    When a screenshot is successfully captured, output 'Wrote [filepath]' before exiting.
    """
    print("Testing $REQ_ID_003: Output Success Message...")
    ...
    timestamp = time.strftime("%Y-%m-%d-%H-%M-%S")
    output_path = os.path.join(tmp_dir, f"{timestamp}_message_test.png")
    ...
    print("✓ $REQ_ID_003: Output Success Message works correctly")
    return True
...
def test_visual_verification():
    ...
    timestamp = time.strftime("%Y-%m-%d-%H-%M-%S")
    output_path = os.path.join(tmp_dir, f"{timestamp}_visual_test.png")
...
def main():
    ...
    try:
        ...
        test_req_id_001_capture_by_id_flag()
        test_req_id_002_save_to_png()
        test_req_id_003_output_success_message()
